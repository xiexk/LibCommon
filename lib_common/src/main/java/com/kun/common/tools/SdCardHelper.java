...
public class SdCardHelper {
   ...
   * 创建文件夹
   */
  public static boolean createFolder(String forder_path) {
    try {
      LogHelper.e("创建文件夹","创建文件夹");
      File destDir_portrait = new File(forder_path);
      if (!destDir_portrait.exists()) {
        return destDir_portrait.mkdirs();
      }
      return true;
    } catch (Exception e) {
      LogHelper.e("创建文件夹","创建文件夹失败");
      LogHelper.e("创建文件夹失败", e.getMessage());
      e.printStackTrace();
      return false;
    }
  }
  ...
  public static void deleteFile(String filePath){
    ...
    LogHelper.i("删除文件", "路径："+filePath);
    File file=SdCardHelper.createFile(filePath);
    assert file != null;
    file.delete();
  }
