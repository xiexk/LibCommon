...
import static me.iwf.photopicker.PhotoPicker.CURT_FIXED_ENABLED;
import static me.iwf.photopicker.PhotoPicker.CURT_FIXED_X;
import static me.iwf.photopicker.PhotoPicker.CURT_FIXED_Y;
...
public class PhotoPickerActivity extends AppCompatActivity {
  ...
  private boolean cutFixed;//固定尺寸裁剪

  private int fixed_x,fixed_y;
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    boolean previewEnabled = getIntent().getBooleanExtra(EXTRA_PREVIEW_ENABLED, true);
     cutFixed = getIntent().getBooleanExtra(CURT_FIXED_ENABLED, false);
     fixed_x = getIntent().getIntExtra(CURT_FIXED_X, ImagePagerFragment.fixed_x);
     fixed_y = getIntent().getIntExtra(CURT_FIXED_Y, ImagePagerFragment.fixed_y);
  ...
  public void addImagePagerFragment(ImagePagerFragment imagePagerFragment) {
    ...
    if(cutFixed){
      imagePagerFragment.setCutProportion(fixed_x,fixed_y);
    }
    getSupportFragmentManager()
        .beginTransaction()
  ...
  protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
    ...
    switch (requestCode) {
      ...
      case ImagePagerFragment.PHOTO_REQUEST_CUT://裁剪返回
        try {

          Uri resuleImage = imagePagerFragment.uritempFile;

      //    String imgurl = SdCardHelper.getImageAbsolutePath(this, resuleImage);
          String imgurl = data.getStringExtra("url");
          Log.v("PhotoPickerActivity", "ImageUrl:" + imgurl);
          pickerFragment.setCropImage(imgurl, imagePagerFragment.getCurrentItem());
          imagePagerFragment.setPhotos(pickerFragment.getPhotoGridAdapter().getCurrentPhotoPaths(), imagePagerFragment.getCurrentItem());
        } catch (Exception e) {
          e.printStackTrace();
        }
        break;
  ...
  public void cropImageBack(String imageUrl){
    Log.v("PhotoPickerActivity", "ImageUrl:" + imageUrl);
    pickerFragment.setCropImage(imageUrl, imagePagerFragment.getCurrentItem());
    imagePagerFragment.setPhotos(pickerFragment.getPhotoGridAdapter().getCurrentPhotoPaths(), imagePagerFragment.getCurrentItem());
  }
