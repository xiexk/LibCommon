...
public class MediaStoreHelper {
  ...
  private final static int MAX_PHOTOS_SIZE=3000;//最大图片展示数量3000
...
//      //todo 测试内容 需要删除
//
//      Photo strAdd= directories.get(0).getPhotos().get(0);
//
//      for(int i=0;i<directories.size();i++){
//        for (int j=0;j<4000;j++) {
//          directories.get(i).getPhotos().add(0, strAdd);
//
//        }}


//      //限制图片数量 太大会导致程序崩溃（activity传递的数据大小超过限制大小错误）
//      List<PhotoDirectory> directoriesCopy = new ArrayList<>();
//      directoriesCopy.addAll(directories);
//      for(int i=0;i<directoriesCopy.size();i++){
//        PhotoDirectory p=directoriesCopy.get(i);
//        if(p.getPhotos().size()>MAX_PHOTOS_SIZE){
//          directories.get(i).setPhotos(p.getPhotos().subList(0,MAX_PHOTOS_SIZE));
//        }
//      }
